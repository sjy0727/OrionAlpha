...
ALTER TABLE `orionalpha`.`inventorysize`
    ADD COLUMN `CashCount` INT(11) NOT NULL DEFAULT 96 AFTER `EtcCount`;

-- Save the currently active pet on the user
ALTER TABLE `orionalpha`.`character`
    ADD COLUMN `PetLockerSN` bigint(20) NOT NULL DEFAULT 0 AFTER `Portal`;
